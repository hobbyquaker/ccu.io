...
function Player(roomName, descriptorUrl, uuid, discovery) {
  ...
  function handleNotification(notification) {
    ...
    xml2js.parseString(notification.body, function (err, result) {
      ...
      switch(eventType) {
        case 'TransportState':
          _this.currentPlayMode.crossfade = result.Event.InstanceID[0].CurrentCrossfadeMode[0].$.val == "1";
          _this.currentPlayMode.repeat = !!(playModesToFlags[result.Event.InstanceID[0].CurrentPlayMode[0].$.val] & 1);
          _this.currentPlayMode.shuffle = !!(playModesToFlags[result.Event.InstanceID[0].CurrentPlayMode[0].$.val] & 2);
          if (result.Event.InstanceID[0].AVTransportURI)
            _this.avTransportUri = result.Event.InstanceID[0].AVTransportURI[0].$.val;
          ...
          _this.state.currentTrack.Uri = result.Event.InstanceID[0].CurrentTrackURI[0].$.val;
          ...
          if (currentMetaData) {
              xml2js.parseString(result.Event.InstanceID[0].CurrentTrackMetaData[0].$.val, function (err, result) {
                  ...
                  // temporary solution for radio title
                  if (result['DIDL-Lite'].item[0]['r:streamContent'] && result['DIDL-Lite'].item[0]['r:streamContent'][0] != "")
                    _this.state.currentTrack.title = result['DIDL-Lite'].item[0]['r:streamContent'][0];
          ...
          _this.state.nextTrack.uri = result.Event.InstanceID[0]['r:NextTrackURI'][0].$.val;
          ...
          if (nextMetaData) {
              xml2js.parseString(result.Event.InstanceID[0]['r:NextTrackMetaData'][0].$.val, function (err, result) {
                  ...
                  _this.state.nextTrack.albumArtURI = (result['DIDL-Lite'].item[0]['upnp:albumArtURI']||[""])[0];
          ...
          break;
        case 'Volume':
          if (result.Event.InstanceID[0].OutputFixed)
            _this.outputFixed = result.Event.InstanceID[0].OutputFixed[0].$.val == "1";
          // if fixed output, we don't care
          if (_this.outputFixed) break;
        ...
        case 'Mute':
          ...
          result.Event.InstanceID[0].Mute.forEach(function (muteState) {
              ...
              if (muteState.$.channel == "Master")
                _this.state.mute = muteState.$.val == "1";
          });

          discovery.emit('mute', {uuid: _this.uuid, state: _this.state.mute});
...
Player.prototype.getFavorites = function (callback) {
  this.soapAction('/MediaServer/ContentDirectory/Control', '"urn:schemas-upnp-org:service:ContentDirectory:1#Browse"', SOAP.GetFavorites, function (success, res) {
    ...
    saxStream.on('text', function (text) {
      if (!this._parser.tag || this._parser.tag.name != "Result") return;
      ...
      xml2js.parseString(responseXML, function (err, result) {
        ...
        result["DIDL-Lite"].item.forEach(function (i) {
          favoritesList.push({
            ...
            albumArtURI: (i["upnp:albumArtURI"]||["/images/browse_missing_album_art.png"])[0]
          });
        });
...
Player.prototype.getQueue = function (startIndex, requestedCount, callback) {
  ...
  this.soapAction('/MediaServer/ContentDirectory/Control', '"urn:schemas-upnp-org:service:ContentDirectory:1#Browse"', soap, function (success, res) {
    ...
    saxStream.on('text', function (text) {
      ...
      var queue = [];
      xml2js.parseString(responseXML, function (err, result) {
        if (!result || !result["DIDL-Lite"] || !result["DIDL-Lite"].item) return false;
        result["DIDL-Lite"].item.forEach(function (i) {
          queue.push({
            title: (i["dc:title"]||[""])[0],
            uri: i.res[0],
